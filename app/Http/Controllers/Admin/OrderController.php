...
class OrderController extends Controller
{
    ...
    public function bulkShip(Request $request)
    {
        ...
        if ($orders->isEmpty()) {
            return back()->with('error', 'Tidak ada pesanan yang valid untuk dikirim.');
        }
        ...
        $message = "Berhasil kirim {$shipped} pesanan.";
        if ($failed > 0) {
            $message .= " Gagal: {$failed} pesanan.";
        }

        return back()->with('success', $message)->with('error_details', $errors);
    }
    ...
    public function bulkPrintLabel(Request $request)
    {
        ...
        foreach ($orders as $order) {
            if (!$order->biteship_order_id) {
                $errors[] = "Order {$order->order_number}: belum memiliki Biteship Order ID.";
                continue;
            }

            $result = $this->biteship->getWaybill($order->biteship_order_id);
            if ($result['success'] && isset($result['pdf_url'])) {
                $labels[] = [
                    'order_number' => $order->order_number,
                    'pdf_url' => $result['pdf_url'],
                ];
            } else {
                $errors[] = "Order {$order->order_number}: " . ($result['error'] ?? 'Gagal mengambil label.');
            }
        }

        if (empty($labels)) {
            return back()->with('error', 'Gagal mengambil semua label. ' . implode(', ', $errors));
        }

        if (count($labels) === 1) {
            return redirect()->away($labels[0]['pdf_url']);
        }
